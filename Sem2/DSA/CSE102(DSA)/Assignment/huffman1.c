...
void compress(char inputFile[], char outputFile[]) {
    ...
    // Writing to the file
    int bitsWritten = 0;
    char outFileHuff[100];
    char outFileTable[100];

    strcpy(outFileHuff, outputFile);
    strcpy(outFileTable, outputFile);

    strcat(outFileHuff, ".huff");
    strcat(outFileTable, ".table");

    fout = fopen(outFileHuff, "w");

    // encoding
    bitsWritten = encode(fin, fout);

    fclose(fin);
    fclose(fout);

    // Writing the huffman table
    fTable = fopen(outFileTable, "w");
    tableWrite(fTable, arr, bitsWritten);
}
...
void decompress(char inputFile[], char outputFile[]) {
    ...
    // File Handling
    char inFileHuff[100];
    char inFileTable[100];

    strcpy(inFileHuff, inputFile);
    strcpy(inFileTable, inputFile);

    strcat(inFileHuff, ".huff");
    strcat(inFileTable, ".table");

    // Reading the table to get the codes
    fTable = fopen(inFileTable, "r");

    // Reading the file
    fscanf(fTable, "%ld", &bitsWritten);

    codeStruct = readTable(fTable);
    fclose(fTable);

    // decoding the file
    fin = fopen(inFileHuff, "r");
